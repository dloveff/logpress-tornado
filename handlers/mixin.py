...
import traceback, sys, os, httplib
import tornado
    ...
    def get_error_html(self, status_code, **kwargs):
        def get_snippet(fp, target_line, num_lines):
            ...
            try:
                with open(fp) as f:
                    ...
                    code = ''.join(all_lines[target_line - half_lines:target_line + half_lines])
        ...
        if self.application.settings.get('debug', False) is False:
            ...
            return self.render_string('errors/exception.html', sys=sys, traceback=traceback, os=os,
                                      get_snippet=get_snippet, exception=exception, status_code=status_code,
                                      kwargs=kwargs)
